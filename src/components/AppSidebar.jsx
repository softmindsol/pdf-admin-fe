...
export function AppSidebar({ 
  ...
  return (
    <Sidebar
      ...
      className={`bg-gradient-to-b from-orange-50 to-orange-100 border-r border-orange-200 transition-all duration-300 fixed left-0 top-0 h-full z-50 ${isVisuallyExpanded ? 'w-64' : 'w-16'}`}
    >
      <SidebarHeader className="p-4 border-b border-orange-200 bg-white/50 backdrop-blur-sm">
        <div className="flex items-center justify-between">
          {isVisuallyExpanded && (
            <div className="flex items-center space-x-2">
              <img src="/logofull.svg" alt="Admin Panel" className="h-8 w-auto" />
            </div>
          )}
          {!isVisuallyExpanded && (
                  ...
                  {item.children ? (
                    <>
                      <SidebarMenuButton
                        ...
                        className={`w-full flex items-center justify-between p-3 rounded-lg transition-all duration-200 group
                          ...
                          ${isExpanded && isVisuallyExpanded ? 'bg-orange-200 text-orange-800' : ''}
                        `}
                        title={!isVisuallyExpanded ? item.label : undefined}
                      >
                        <div className="flex items-center space-x-3">
                          <Icon size={20} className="flex-shrink-0" />
                          {isVisuallyExpanded && <span className="font-medium">{item.label}</span>}
                        </div>
                        {isVisuallyExpanded && (
                                ...
                                <SidebarMenuItem key={childIndex}>
                                  <SidebarMenuButton
                                    ...
                                    className={`w-full flex items-center space-x-3 p-2.5 rounded-md text-sm
                                      ...
                                      ${isChildActive ? 'bg-orange-200 text-orange-800 font-semibold' : ''}
                                    `}
                                  >
                    ...
                    <SidebarMenuButton
                      ...
                      className={`w-full flex items-center p-3 rounded-lg transition-all duration-200
                        ...
                        hover:bg-orange-200 hover:text-orange-800 text-orange-700
                        ${isActive && !item.children ? 'bg-orange-200 text-orange-800 font-semibold' : ''}
                      `}
                      title={!isVisuallyExpanded ? item.label : undefined}
                    >
                      <Icon size={20} className="flex-shrink-0" />
       ...
       <SidebarFooter className="p-4 border-t border-orange-200 bg-white/30 backdrop-blur-sm">
        {isVisuallyExpanded ? (
          <div className="text-center">
            <p className="text-xs text-orange-600">© 2024 Admin Panel</p>
