...
export default function LoginPage() {
  // 1. Destructure all necessary states and the reset function from the hook
  const [login, { data, error, isLoading, isSuccess, isError, reset }] = useLoginMutation();
  ...
  // 4. Use a separate useEffect to react to the 'isError' state change
  useEffect(() => {
    if (isError && error) {
  ...
  // 5. (Optional but recommended) Reset mutation state when the component unmounts
  useEffect(() => {
    return () => {
      reset();
    };
